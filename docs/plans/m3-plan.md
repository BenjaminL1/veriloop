...
node -e 'const c=require("/tmp/vl-bench/mined.json").candidates; process.exit(c.every(r=>r.citations.length>=2 && typeof r.conformance==="number")?0:1)'; echo $?  # → 0
```
